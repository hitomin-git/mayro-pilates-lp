...
frame.addEventListener('load',()=>{const doc=frame.contentDocument;doc.querySelector('.review-nav')?.remove();doc.querySelector('.design-navigation')?.remove();doc.body.style.paddingBottom='0';sizePreview();Promise.all(Array.from(doc.images).map(i=>i.decode().catch(()=>{}))).then(sizePreview);});
