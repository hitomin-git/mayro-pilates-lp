...
const pages=['final.html','versions/change4.html','index.html','versions/old.html','versions/change1.html','versions/change2.html','versions/review2.html','versions/change3.html','versions/change3-price.html'];
const failures=[];
...
for(const name of ['final.css','comparison.css','versions/old.css','versions/change1.css','versions/change2.css','versions/review2.css']){const file=path.join(root,name);for(const [,ref]of fs.readFileSync(file,'utf8').matchAll(/url\(['"]?([^)'"\s]+)['"]?\)/g))checkRef(file,ref);}
